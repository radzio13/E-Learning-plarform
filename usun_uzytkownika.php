...
<body id="page-top">
	...
	<div id="main">
					...
					<br>
					<h1>Panel usuwania użytkownika</h1>
					<h2>Jeżeli chcesz usunąć użytkownika, wypełnij poniższe pola i kliknij przycisk.</h2>
							
						<form method="POST" action="usun_uzytkownika.php#main">
							...
							<input type="submit" value="Usuń użytkownika" name="usun_uzytk">
						</form>
				...
				$sql = "SELECT login FROM uzytkownicy WHERE login='$login' OR id_uzytkownika='$id_uzytkownika'";
				$result = mysqli_query($connection,$sql);

				if(mysqli_num_rows($result) != 0)
				{

						if (!empty($id_uzytkownika) && !empty($login)) 
						{ 
							
							$query = "DELETE FROM uzytkownicy WHERE login='$login' AND id_uzytkownika='$id_uzytkownika'";
							
							$result = mysqli_query($connection, $query);

							 echo "Konto zostało usunięte!";
						}
						else echo "Nie wpisałeś ID lub loginu!";
				}
