...
session_start();
	...
	if($_SESSION['zalogowani']==true)
	{
		...
		if(empty($Tresc) || empty($ocena))
		{
			...
			header('Location:kurs3.php#dodaj_kom');
		}
		else
		{
			$sql = "INSERT INTO komentarze(id_kursu, login, tresc_kom, ocena) VALUES (3, '$Login', '$Tresc', '$ocena')";
			
			if ($mysqli->query($sql) === TRUE) 
			{
				$_SESSION['dodano_komentarz']='<span style="color:white">Pomyślnie dodano komentarz!</span>'; 
				header('Location:kurs3.php#dodaj_kom');
				$sql2 = "UPDATE oceny SET ocena = (SELECT avg(ocena) FROM komentarze) WHERE id_kursu=3";
